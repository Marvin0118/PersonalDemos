...
- (void)viewDidLoad
{
    ...
    self.imageView = [[UIImageView alloc]initWithFrame:CGRectMake(20, 200, 240, 450)];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if (sock==serverSocket) {
        if (data.length == 4) {
            ...
            self.length = value;
            NSLog(@"===>%zd",value);
            [self.mData setData:[NSData data]];
        }else{
            [self.mData appendData:data];
            NSLog(@":::>%zd",self.mData.length);
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.mData.length == self.length) {
                    [self.imageView setImage:[UIImage imageWithData:self.mData]];
                }
            });
        }
    }
