...
+ (void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        /**
         交换后的方法和实现
         */
        SEL swizzWillSel = @selector(mr_viewWillAppear:);
        ...
        SEL swizzDisSel = @selector(mr_viewWillDisappear:);
...
- (void)mr_viewWillDisappear:(BOOL)animated{
    ...
    if (![arrBanList containsObject:className]) {
        ...
        NSLog(@"页面消失:%@  停留时间:%@ms",className,self.duration);
        
    }
    [self mr_viewWillDisappear:animated];
}
